...
#define TINYNPY_MINOR_VERSION 1
...
public:
	...
	std::vector<float> FloatVector() const {
		if (type == 'f' && wordSize == 4) return DataVector<float>();
		return std::vector<float>();
	}
	std::vector<double> DoubleVector() const {
		if (type == 'f' && wordSize == 8) return DataVector<double>();
		return std::vector<double>();
	}
	std::vector<std::string> StringVector() const;

	// get info string about the array
	std::string PrintInfo(const std::string& name = std::string()) const;
